...
-   **Monitor only**: In this mode, watchtower checks for container updates, sends notifications and invokes the [pre-check/post-check hooks](lifecycle-hooks.md) on the containers but does **not** perform the update.
...
When the label is specified on a container, watchtower treats that container exactly as if [`WATCHTOWER_MONITOR_ONLY`](arguments.md#without-updating-containers) was set, but the effect is limited to the individual container. 
